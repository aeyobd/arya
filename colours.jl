...
# ╔═╡ 73854ce9-e3cf-460c-a6c0-0d65ddd8a078
using Colors, ColorTypes, ColorSchemes, ColorSchemeTools  # ColorVectorSpace

# ╔═╡ a2cc7815-0a50-4feb-9f26-aade2062dce0
md"""
# Color blind friendly color design
...
- summary of CV simulations https://daltonlens.org/colorblindness-simulator 
- Brettel 1997 (original paper)
- https://ieeexplore.ieee.org/document/5290741

## Other examples
- SRON colour palate https://personal.sron.nl/~pault/
- Scientific colour maps (in ColorSchemes.jl)
- PerceptualColourMaps
"""

# ╔═╡ 1443baa0-938d-4e5b-9dcf-20e54e6a4b3d
...
# ╔═╡ e714b3f9-1496-45d4-8f62-e84953fbe662
show(convert(XYZ, convert(RGB, Oklch(0.5, 0.2, 2.1))))

# ╔═╡ 1054d5e6-7558-42b6-a50e-b911aa5eb3da
md"""
...
md"""
lightness:  $(@bind l PlutoUI.Slider(0:0.01:1, default=0.5))
chroma: $(@bind c PlutoUI.Slider(0:0.002:0.2, default=0.15))
hue: $(@bind h PlutoUI.Slider(0:0.1:360, default=300))
"""

# ╔═╡ 98f1f812-2872-44ab-a646-2dca8c1a96d6
md"""
lightness:  $(@bind l2 Slider(0:0.01:1, default=0.9))
chroma: $(@bind c2 Slider(0:0.002:0.2, default=0.07))
hue: $(@bind h2 Slider(0:0.1:360, default=100))
"""

# ╔═╡ d74e23c2-721d-43d5-b9f4-a76074025fd5
md"""
lightness:  $(@bind l4 Slider(0:0.01:1, default=0.12))
...
begin
	...
	Converts a colour to RGB, but if is outside colourspace, returns bright red
	"""
...
"""
Safely converts a colour to RGB, truncating RGB values to 1 (so not perfect)
"""
...
# ╔═╡ 6a1e6434-f7b9-464f-9d42-516a04c3aef2
x = LinRange(0.2, 1, 1000)

# ╔═╡ f5e1d47b-bde1-4b86-a9ee-bed3d824482e
tau = 2

# ╔═╡ 802d88f0-7fc5-4229-91c1-e998dfdfe85a
begin 
	y = @. (1 - exp(-x * tau)) / (1 - exp(-tau))
	y = @. (x)
end

# ╔═╡ 3e45ade3-d603-48c0-a26a-6920d3405817
lines(x, y)

# ╔═╡ db143104-0d90-4011-8d04-69a374b48876
@. mean_rgb.(Oklch.(y, 0., 0.))

# ╔═╡ b1aacbbf-8458-47f8-9413-868d393d189d
"""
	huerange(colour1, colour2, N; [increasing])

creates a interpolated range of colours between two values. 
"""
...
mean_rgb.(huerange(Oklch(l3, c3, h3), Oklch(1., c3, h3), 256, increasing=false))

# ╔═╡ c5aa50d4-25bf-47f5-8ca6-9d93a280f6e9
mean_rgb(Oklch(l3, 0., 0.)).b

# ╔═╡ 83ccf32e-9ccc-4dcd-b146-16c023a92527
l3

# ╔═╡ 8e5e2d51-1c54-494c-9fec-21ff369762cd
rainbow = mean_rgb.(huerange(Oklch(0.8, 0.08, 0.), Oklch(0.8, 0.1, 360.), 256))
...
begin
	function protanize(c::RGB, strength=1)
		lms = convert(LMS, c)
		l = lms.l*(1-strength) + strength*lms.m
		lms1 = LMS(l, lms.m, lms.s)
		return convert(RGB, lms1)
	end
	function deuterize(c::RGB, strength=1)
		lms = convert(LMS, c)
		m = lms.m*(1-strength) + strength*lms.l

		lms1 = LMS(lms.l, m, lms.s)
		return convert(RGB, lms1)
	end
	function tritanize(c::RGB, strength=1)
		lms = convert(LMS, c)
		s = lms.s * (1-strength) + strength*lms.m
		lms1 = LMS(lms.l, lms.m, s)
		return convert(RGB, lms1)
	end
end

# ╔═╡ a59b1db0-5433-4d5a-a44a-3acd6effb54f
t1 = tritanize.(cmap1, 1)

# ╔═╡ ca93715b-181d-4a69-8998-76dc949967b6
cmap = cmap1

# ╔═╡ 929658b1-e7fc-4473-a9b2-b09874a17821
cmap_p = protanize.(cmap)

# ╔═╡ f524b54b-82ae-4e95-8440-1a25e8a421a1
cmap_d = deuterize.(cmap)

# ╔═╡ df79cb39-5dfc-4ebd-8c0d-7894561acebd
cmap_t = tritanize.(cmap)

# ╔═╡ 00bb7106-d867-4d12-96e5-f336abade1fb
begin 
	labels = ["all", "prot", "deut", "trit"]

	cmaps = [cmap, cmap_p, cmap_d, cmap_t]
end

# ╔═╡ c5aa5766-49f2-4a33-9bd7-4c4ec0b534b1
let
	fig = Figure()
	ax1 = Axis(fig[1, 1], ylabel="lum")
	ls = []
	for (m, label) in zip(cmaps, labels)
		okc = convert.(Oklch, m)
		l = [o.l for o in okc]
		c = [o.c for o in okc]
		push!(ls, lines!(ax1, l, label=label))
	end
	Legend(fig[1, 2], ls, labels)

	fig
end

# ╔═╡ 27a14f0d-b6b3-402b-b7ad-dbfed767d727
let
	fig = Figure()
	ax1 = Axis(fig[1, 1], ylabel="chroma")
	ls = []
	for (m, label) in zip(cmaps, labels)
		okc = convert.(Oklch, m)
		c = [o.c for o in okc]
		push!(ls, lines!(ax1, c, label=label))
	end
	Legend(fig[1, 2], ls, labels)
...
# ╔═╡ fa283508-68fb-467a-97a2-8751bf0f1517
md"""
# Finite element palletes

This part is much harder as there are far more ways to chose many different colors
"""

# ╔═╡ 42a42754-bbca-45c9-823e-a11dfa39bf36
begin
pc1 = colorant"#0173b2"
	pc2 = colorant"#de8f05"
	pc3 = colorant"#029e73"
	...
	pc8 = colorant"#949494"
	pc9 = colorant"#ece133"
	pc10 = colorant"#56b4e9"
	pal1 = [pc1, pc2, pc3, pc4, pc5, pc6, pc7, pc8, pc9, pc10]
end

# ╔═╡ adebd546-45bd-4d01-b687-d5fff723bfbe
function plot_palette_slices(pal)
	okp = convert.(Oklch, pal)
	c = [o.c for o in okp]
	l = [o.l for o in okp]
	h = deg2rad.([o.h for o in okp])

	kwargs = Dict(:mc=>pal, :ms=>10, :msw=>0)
	pwargs = Dict(:proj=>:polar, :xticks=>[])
	p1 = scatter(l, c; xlabel="lightness", ylabel="chroma", kwargs...)
	p2 =  scatter(h, l; title="lightness",pwargs..., kwargs...)
	p3 = scatter(h, c;  title="chroma", pwargs..., kwargs...)
...
# ╔═╡ 081aecce-c1a0-4235-aed4-cbd9f2af2f76
plot_palette_slices(pal1)

# ╔═╡ 95741d02-ec47-406a-b353-d3179bfc012a
function sim_cvd_palette(pal)
	for i in 1:4
		cv = [x->x, protanize, deuterize, tritanize][i]
		label = ["normal", "pro", "deu", "tri"][i]
		pal_t = cv.(pal)
		okp = convert.(Oklch, pal_t)
		c = [o.c for o in okp]
		l = [o.l for o in okp]
		h = deg2rad.([o.h for o in okp])
		h = mod.(h, 2π)
		...
		if label ∈ ["pro", "deu"]
			@. c *= ifelse(h>π, 1, -1)
		elseif label == "tri"
			@. c *= ifelse(π/2 < h < 3π/2, 1, -1)
		end

		@info "$label" scatter(c, l; xlabel="chroma", ylabel="lightness", mc=pal_t, ms=10, msw=0)
	end
	return
end

# ╔═╡ 7ab03f27-9e19-454a-8fa0-a1af110cef72
palette = pal1

# ╔═╡ 97022847-a193-4d45-ad74-c2600e3f3f04
palette_p = protanize.(palette)

# ╔═╡ 5fda0376-a89f-4043-94ae-61b6a207207d
palette_d = deuterize.(palette)

# ╔═╡ d3871fef-e2f4-45c5-a73f-083722f06a8f
palette_t = tritanize.(palette)

# ╔═╡ 1754345e-4832-4636-a8d3-86868b0c30a9
palette_ok = convert.(Oklch, palette)
...
manifest_format = "2.0"
project_hash = "bf270c2f2960166b168edba3a4451a34d7e63430"
...
[[deps.ColorSchemeTools]]
deps = ["Clustering", "ColorSchemes", "Colors", "FileIO", "Images", "Interpolations", "QuartzImageIO"]
git-tree-sha1 = "4ff1579f2c7c5f52d741d519b4b93f5ff7bb040e"
