...
using ColorTypes, Colors, PlutoUI, ColorVectorSpace, ColorSchemes, Plots
...
md"""
lightness:  $(@bind l Slider(0:0.01:1, default=0.5))
chroma: $(@bind c Slider(0:0.002:0.2, default=0.15))
hue: $(@bind h Slider(0:0.1:360, default=300))
"""
...
# ╔═╡ 5d97d65b-0e1d-4035-921d-c563f709f100
function check_cv(cmap)
	p = protanize.(cmap)
	d = deuterize.(cmap)
	t = tritanize.(cmap)
	...
	for plt in [pl0, pl1, pl2, pl3, p1, p2]
		@info "plot" plt
	end
...
rainbow = mean_rgb.(huerange(Oklch(0.8, 0.1, 0.), Oklch(0.8, 0.1, 360.), 256))

# ╔═╡ 0e32ee55-f320-4220-b12f-d4f59c9a2cf9
check_cv(rainbow)

# ╔═╡ 034abd2c-f104-4192-b6bf-9e9e6ac180ff
check_cv(cmap1)

# ╔═╡ 7817690a-1559-4c56-b06b-b6bf22f5f093
check_cv(cmap2)

# ╔═╡ 98d465e3-a233-40c7-ab7e-b033f0d0cc65
check_cv(cmap3)

# ╔═╡ dea49737-5c68-4f6c-bc68-f3bc5ffd784c
check_cv(cmap4)
...
# ╔═╡ 97db4be5-10d7-42e7-a01e-0ea4709cd986
@bind in_col ColorPicker()

# ╔═╡ 59a02795-e79a-4090-a67f-5b146e10d699
begin
in_ok = convert(Oklch, in_col);
print(in_ok)
end

# ╔═╡ 42a42754-bbca-45c9-823e-a11dfa39bf36
begin
pc1 = Oklch(0.7989825306743341,0.08779532230780199,-28.12411910878562)
pc2 = Oklch(0.7663755725896391,0.07581717842089754,148.85207985717798)
pc3 = Oklch(0.7658389054707755,0.09853552919115345,60.79673554068782)
pc4 = Oklch(0.7, 0.0, 0.5)
pc5 = Oklch(0., 0., 0.)
	pal1 = mean_rgb.([pc1, pc2, pc3, pc4, pc5])
end
...
# ╔═╡ 804c8433-5eec-4dbb-8639-136206ead2f0
plot_palette_slices(pal1)

# ╔═╡ 885ec0a3-9bd3-4ee0-8b75-70f82bb0ad2e
begin
	for f in [x->x, protanize, deuterize, tritanize]
		pal_c = f.(pal1)
		plot_palette_slices(pal_c)
	end
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "547ee8c1b556eba6a560abcb5958ce933cc8da78"
